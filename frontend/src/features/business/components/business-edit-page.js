...
export function BusinessEditPage() {
  ...
  async function fetchBusinessDetail(businessId) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("사업 정보를 불러오지 못했습니다.");
      }
      ...
      setNodeCount(String(data.nodeCount ?? 0));
      setCoreCount(String(data.coreCount ?? 0));
      setStatus(resolveStatusValue(data.status));
  ...
  function resolveStatusValue(label) {
    const statusMap = {
      "대기 중": "standby",
      "고객 미팅": "meeting",
      PoC: "poc",
    ...
    return statusMap[label] || label || "standby";
  }
  ...
  async function handleSubmit(event) {
    ...
    if (!window.confirm("사업 정보를 수정하시겠습니까?")) {
      return;
    }
    ...
    try {
      ...
      await router.push(`/businesses/${id}`);
    } catch (submitError) {
  ...
  return (
    <AdminLayout activePath="/business" title="사업 수정" actions={null}>
      <form className="license-create-card" onSubmit={handleSubmit}>
        {isLoading ? <p className="form-feedback">사업 정보를 불러오는 중입니다.</p> : null}
          ...
          <div className="license-create-grid">
            ...
            <label className="form-field">
              ...
              <select className="form-control" value={managerId} onChange={(event) => setManagerId(event.target.value)}>
                ...
                {(business.managerOptions || []).map((option) => (
            ...
            <label className="form-field">
              ...
              <select className="form-control" value={customerId} onChange={(event) => setCustomerId(event.target.value)}>
                <option value="">고객회사 선택</option>
                {(business.customerOptions || []).map((option) => (
                  <option key={option.id} value={option.id}>
                    {option.label}
                  </option>
                ))}
            ...
            <label className="form-field">
              ...
              <select className="form-control" value={productId} onChange={(event) => setProductId(event.target.value)}>
                ...
                {(business.productOptions || []).map((option) => (
                  <option key={option.id} value={option.id}>
                    {option.label}
                  </option>
                ))}
            ...
            <label className="form-field business-details-field">
              ...
              <textarea className="detail-textarea detail-textarea-edit" value={details} onChange={(event) => setDetails(event.target.value)} />
            </label>
          </div>
        ) : null}
        ...
        <div className="form-actions">
          <Link href={id ? `/businesses/${id}` : "/business"} legacyBehavior>
