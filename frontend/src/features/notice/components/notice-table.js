...
export function NoticeTable({ notices, isLoading, error, pagination, onPageChange }) {
  const router = useRouter();
  const startNumber = pagination.total - (pagination.page - 1) * pagination.limit;

  function handleRowSelect(noticeId) {
    void router.push(`/notices/${noticeId}`);
  }

  return (
    <section className="table-card">
      <div className="table-scroll">
        <table className="license-table notice-table">
          <thead>
            <tr>
              <th>NO</th>
              <th>제목</th>
            ...
            {isLoading ? (
              ...
              notices.map((notice, index) => (
                <tr
                  ...
                  role="button"
                  tabIndex={0}
                  onClick={() => handleRowSelect(notice.id)}
                  onKeyDown={(event) => {
                    if (event.key === "Enter" || event.key === " ") {
                      event.preventDefault();
                      handleRowSelect(notice.id);
                    }
                  }}
                >
                  <td className="cell-number">{startNumber - index}</td>
                  <td className="cell-notice-title">{notice.title}</td>
                  <td className="cell-notice-writer">{notice.writer}</td>
                  <td className="cell-notice-level">{notice.level}</td>
                  <td className="cell-partner-created">{formatDateTime(notice.createdAt)}</td>
                </tr>
              ))
            )}
          </tbody>
