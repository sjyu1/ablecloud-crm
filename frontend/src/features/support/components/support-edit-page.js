...
export function SupportEditPage() {
  ...
  const filteredBusinessOptions = useMemo(
    () =>
      (support?.businessOptions || []).filter(
        (option) => !customerId || String(option.customerId) === String(customerId)
      ),
    [support?.businessOptions, customerId]
  );
  ...
  async function fetchSupportDetail(supportId) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("기술지원 정보를 불러오지 못했습니다.");
      }
      ...
      setActionType(data.actionType || "phone");
      setManager(data.manager || "");
      setStatus(data.status || "processing");
      setRequester(data.requester || "");
      setRequesterTelnum(data.requesterTelnum || "");
  ...
  async function handleSubmit(event) {
    ...
    if (!window.confirm("기술지원 정보를 수정하시겠습니까?")) {
      return;
    }
  ...
  return (
    <AdminLayout activePath="/support" title="기술지원 수정" actions={null}>
      <form className="license-create-card" onSubmit={handleSubmit}>
        {isLoading ? <p className="form-feedback">기술지원 정보를 불러오는 중입니다.</p> : null}
        {!isLoading && support ? (
          <div className="license-create-grid support-form-grid">
            <label className="form-field">
              ...
              <select className="form-control" value={customerId} onChange={handleCustomerChange}>
                ...
                {(support.customerOptions || []).map((option) => (
        ...
        <div className="form-actions">
          <Link href={id ? `/supports/${id}` : "/support"} legacyBehavior>
            <a className="secondary-button">취소</a>
          </Link>
          <button className="primary-button" type="submit" disabled={isLoading || isSubmitting || !support}>
